...
Add a little more text and context to your [GNOME](https://www.gnome.org/) user experience: Window icon & title, custom clock, user/host name, and lock screen message.

## Features

Creates a top bar “context” button with the focused window’s icon and title, or an app grid icon if no window is focused. Optionally shows a clock with advanced display options, the user and/or host name on the system menu button, and a notification message on the lock screen.
